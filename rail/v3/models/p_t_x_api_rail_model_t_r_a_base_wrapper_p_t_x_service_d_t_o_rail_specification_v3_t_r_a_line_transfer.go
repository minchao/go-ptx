...
func (m *PTXAPIRailModelTRABaseWrapperPTXServiceDTORailSpecificationV3TRALineTransfer) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateLineTransfers(formats); err != nil {
		res = append(res, err)
	}
...
func (m *PTXAPIRailModelTRABaseWrapperPTXServiceDTORailSpecificationV3TRALineTransfer) validateLineTransfers(formats strfmt.Registry) error {
	...
	for i := 0; i < len(m.LineTransfers); i++ {
		if swag.IsZero(m.LineTransfers[i]) { // not required
			continue
		}

		if m.LineTransfers[i] != nil {
			if err := m.LineTransfers[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("LineTransfers" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("LineTransfers" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
