...
func (m *PTXAPIRailModelTRABaseWrapperPTXServiceDTORailSpecificationV3TRAStationExitStationExit) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateStationExits(formats); err != nil {
		res = append(res, err)
	}
...
func (m *PTXAPIRailModelTRABaseWrapperPTXServiceDTORailSpecificationV3TRAStationExitStationExit) validateStationExits(formats strfmt.Registry) error {
	...
	for i := 0; i < len(m.StationExits); i++ {
		if swag.IsZero(m.StationExits[i]) { // not required
			continue
		}

		if m.StationExits[i] != nil {
			if err := m.StationExits[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("StationExits" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("StationExits" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
