// Code generated by go-swagger; DO NOT EDIT.

package t_r_a

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/minchao/go-ptx/rail/v2/models"
)

// TRAAPILine2142Reader is a Reader for the TRAAPILine2142 structure.
type TRAAPILine2142Reader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *TRAAPILine2142Reader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewTRAAPILine2142OK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 299:
		result := NewTRAAPILine2142Status299()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 304:
		result := NewTRAAPILine2142NotModified()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result
	default:
		return nil, runtime.NewAPIError("response status code does not match any response statuses defined for this endpoint in the swagger spec", response, response.Code())
	}
}

// NewTRAAPILine2142OK creates a TRAAPILine2142OK with default headers values
func NewTRAAPILine2142OK() *TRAAPILine2142OK {
	return &TRAAPILine2142OK{}
}

/* TRAAPILine2142OK describes a response with status code 200, with default header values.

Success
*/
type TRAAPILine2142OK struct {
	Payload []*models.PTXServiceDTORailSpecificationV2TRALine
}

func (o *TRAAPILine2142OK) Error() string {
	return fmt.Sprintf("[GET /v2/Rail/TRA/Line][%d] tRAApiLine2142OK  %+v", 200, o.Payload)
}
func (o *TRAAPILine2142OK) GetPayload() []*models.PTXServiceDTORailSpecificationV2TRALine {
	return o.Payload
}

func (o *TRAAPILine2142OK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewTRAAPILine2142Status299 creates a TRAAPILine2142Status299 with default headers values
func NewTRAAPILine2142Status299() *TRAAPILine2142Status299 {
	return &TRAAPILine2142Status299{}
}

/* TRAAPILine2142Status299 describes a response with status code 299, with default header values.

加入參數'?health=true'即可查詢此API服務的健康狀態
*/
type TRAAPILine2142Status299 struct {
	Payload *models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth
}

func (o *TRAAPILine2142Status299) Error() string {
	return fmt.Sprintf("[GET /v2/Rail/TRA/Line][%d] tRAApiLine2142Status299  %+v", 299, o.Payload)
}
func (o *TRAAPILine2142Status299) GetPayload() *models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth {
	return o.Payload
}

func (o *TRAAPILine2142Status299) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewTRAAPILine2142NotModified creates a TRAAPILine2142NotModified with default headers values
func NewTRAAPILine2142NotModified() *TRAAPILine2142NotModified {
	return &TRAAPILine2142NotModified{}
}

/* TRAAPILine2142NotModified describes a response with status code 304, with default header values.

服務端會在Response加上Last-Modified header，表示最近的更新時間。客戶端能利用此時間，於Request加上If-Modified-Since header，若沒有更新，服務端會回應304 StatusCode且空值Content
*/
type TRAAPILine2142NotModified struct {
}

func (o *TRAAPILine2142NotModified) Error() string {
	return fmt.Sprintf("[GET /v2/Rail/TRA/Line][%d] tRAApiLine2142NotModified ", 304)
}

func (o *TRAAPILine2142NotModified) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}
