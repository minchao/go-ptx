// Code generated by go-swagger; DO NOT EDIT.

package t_r_a

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/minchao/go-ptx/rail/v2/models"
)

// TRAAPIShape2145Reader is a Reader for the TRAAPIShape2145 structure.
type TRAAPIShape2145Reader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *TRAAPIShape2145Reader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewTRAAPIShape2145OK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 299:
		result := NewTRAAPIShape2145Status299()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 304:
		result := NewTRAAPIShape2145NotModified()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result
	default:
		return nil, runtime.NewAPIError("response status code does not match any response statuses defined for this endpoint in the swagger spec", response, response.Code())
	}
}

// NewTRAAPIShape2145OK creates a TRAAPIShape2145OK with default headers values
func NewTRAAPIShape2145OK() *TRAAPIShape2145OK {
	return &TRAAPIShape2145OK{}
}

/* TRAAPIShape2145OK describes a response with status code 200, with default header values.

Success
*/
type TRAAPIShape2145OK struct {
	Payload []*models.PTXServiceDTORailSpecificationV2TRATRAShape
}

func (o *TRAAPIShape2145OK) Error() string {
	return fmt.Sprintf("[GET /v2/Rail/TRA/Shape][%d] tRAApiShape2145OK  %+v", 200, o.Payload)
}
func (o *TRAAPIShape2145OK) GetPayload() []*models.PTXServiceDTORailSpecificationV2TRATRAShape {
	return o.Payload
}

func (o *TRAAPIShape2145OK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewTRAAPIShape2145Status299 creates a TRAAPIShape2145Status299 with default headers values
func NewTRAAPIShape2145Status299() *TRAAPIShape2145Status299 {
	return &TRAAPIShape2145Status299{}
}

/* TRAAPIShape2145Status299 describes a response with status code 299, with default header values.

加入參數'?health=true'即可查詢此API服務的健康狀態
*/
type TRAAPIShape2145Status299 struct {
	Payload *models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth
}

func (o *TRAAPIShape2145Status299) Error() string {
	return fmt.Sprintf("[GET /v2/Rail/TRA/Shape][%d] tRAApiShape2145Status299  %+v", 299, o.Payload)
}
func (o *TRAAPIShape2145Status299) GetPayload() *models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth {
	return o.Payload
}

func (o *TRAAPIShape2145Status299) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewTRAAPIShape2145NotModified creates a TRAAPIShape2145NotModified with default headers values
func NewTRAAPIShape2145NotModified() *TRAAPIShape2145NotModified {
	return &TRAAPIShape2145NotModified{}
}

/* TRAAPIShape2145NotModified describes a response with status code 304, with default header values.

服務端會在Response加上Last-Modified header，表示最近的更新時間。客戶端能利用此時間，於Request加上If-Modified-Since header，若沒有更新，服務端會回應304 StatusCode且空值Content
*/
type TRAAPIShape2145NotModified struct {
}

func (o *TRAAPIShape2145NotModified) Error() string {
	return fmt.Sprintf("[GET /v2/Rail/TRA/Shape][%d] tRAApiShape2145NotModified ", 304)
}

func (o *TRAAPIShape2145NotModified) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}
