// Code generated by go-swagger; DO NOT EDIT.

package air

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/minchao/go-ptx/air/v2/models"
)

// AirAPIFlight2014Reader is a Reader for the AirAPIFlight2014 structure.
type AirAPIFlight2014Reader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *AirAPIFlight2014Reader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewAirAPIFlight2014OK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 299:
		result := NewAirAPIFlight2014Status299()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 304:
		result := NewAirAPIFlight2014NotModified()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result
	default:
		return nil, runtime.NewAPIError("response status code does not match any response statuses defined for this endpoint in the swagger spec", response, response.Code())
	}
}

// NewAirAPIFlight2014OK creates a AirAPIFlight2014OK with default headers values
func NewAirAPIFlight2014OK() *AirAPIFlight2014OK {
	return &AirAPIFlight2014OK{}
}

/* AirAPIFlight2014OK describes a response with status code 200, with default header values.

Success
*/
type AirAPIFlight2014OK struct {
	Payload []*models.PTXServiceDTOAirSpecificationV2FIDS
}

func (o *AirAPIFlight2014OK) Error() string {
	return fmt.Sprintf("[GET /v2/Air/FIDS/Flight][%d] airApiFlight2014OK  %+v", 200, o.Payload)
}
func (o *AirAPIFlight2014OK) GetPayload() []*models.PTXServiceDTOAirSpecificationV2FIDS {
	return o.Payload
}

func (o *AirAPIFlight2014OK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewAirAPIFlight2014Status299 creates a AirAPIFlight2014Status299 with default headers values
func NewAirAPIFlight2014Status299() *AirAPIFlight2014Status299 {
	return &AirAPIFlight2014Status299{}
}

/* AirAPIFlight2014Status299 describes a response with status code 299, with default header values.

加入參數'?health=true'即可查詢此API服務的健康狀態
*/
type AirAPIFlight2014Status299 struct {
	Payload *models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth
}

func (o *AirAPIFlight2014Status299) Error() string {
	return fmt.Sprintf("[GET /v2/Air/FIDS/Flight][%d] airApiFlight2014Status299  %+v", 299, o.Payload)
}
func (o *AirAPIFlight2014Status299) GetPayload() *models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth {
	return o.Payload
}

func (o *AirAPIFlight2014Status299) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewAirAPIFlight2014NotModified creates a AirAPIFlight2014NotModified with default headers values
func NewAirAPIFlight2014NotModified() *AirAPIFlight2014NotModified {
	return &AirAPIFlight2014NotModified{}
}

/* AirAPIFlight2014NotModified describes a response with status code 304, with default header values.

服務端會在Response加上Last-Modified header，表示最近的更新時間。客戶端能利用此時間，於Request加上If-Modified-Since header，若沒有更新，服務端會回應304 StatusCode且空值Content
*/
type AirAPIFlight2014NotModified struct {
}

func (o *AirAPIFlight2014NotModified) Error() string {
	return fmt.Sprintf("[GET /v2/Air/FIDS/Flight][%d] airApiFlight2014NotModified ", 304)
}

func (o *AirAPIFlight2014NotModified) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}
