// Code generated by go-swagger; DO NOT EDIT.

package city_bus

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"

	"github.com/minchao/go-ptx/bus/v2/models"
)

// CityBusAPINews2044Reader is a Reader for the CityBusAPINews2044 structure.
type CityBusAPINews2044Reader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *CityBusAPINews2044Reader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewCityBusAPINews2044OK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 299:
		result := NewCityBusAPINews2044Status299()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	case 304:
		result := NewCityBusAPINews2044NotModified()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return nil, result
	default:
		return nil, runtime.NewAPIError("response status code does not match any response statuses defined for this endpoint in the swagger spec", response, response.Code())
	}
}

// NewCityBusAPINews2044OK creates a CityBusAPINews2044OK with default headers values
func NewCityBusAPINews2044OK() *CityBusAPINews2044OK {
	return &CityBusAPINews2044OK{}
}

/* CityBusAPINews2044OK describes a response with status code 200, with default header values.

Success
*/
type CityBusAPINews2044OK struct {
	Payload []*models.PTXServiceDTOBusSpecificationV2BusNews
}

func (o *CityBusAPINews2044OK) Error() string {
	return fmt.Sprintf("[GET /v2/Bus/News/City/{City}][%d] cityBusApiNews2044OK  %+v", 200, o.Payload)
}
func (o *CityBusAPINews2044OK) GetPayload() []*models.PTXServiceDTOBusSpecificationV2BusNews {
	return o.Payload
}

func (o *CityBusAPINews2044OK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewCityBusAPINews2044Status299 creates a CityBusAPINews2044Status299 with default headers values
func NewCityBusAPINews2044Status299() *CityBusAPINews2044Status299 {
	return &CityBusAPINews2044Status299{}
}

/* CityBusAPINews2044Status299 describes a response with status code 299, with default header values.

加入參數'?health=true'即可查詢此API服務的健康狀態
*/
type CityBusAPINews2044Status299 struct {
	Payload *models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth
}

func (o *CityBusAPINews2044Status299) Error() string {
	return fmt.Sprintf("[GET /v2/Bus/News/City/{City}][%d] cityBusApiNews2044Status299  %+v", 299, o.Payload)
}
func (o *CityBusAPINews2044Status299) GetPayload() *models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth {
	return o.Payload
}

func (o *CityBusAPINews2044Status299) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PTXServiceDTOSharedSpecificationV3BaseDisplayHealth)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewCityBusAPINews2044NotModified creates a CityBusAPINews2044NotModified with default headers values
func NewCityBusAPINews2044NotModified() *CityBusAPINews2044NotModified {
	return &CityBusAPINews2044NotModified{}
}

/* CityBusAPINews2044NotModified describes a response with status code 304, with default header values.

服務端會在Response加上Last-Modified header，表示最近的更新時間。客戶端能利用此時間，於Request加上If-Modified-Since header，若沒有更新，服務端會回應304 StatusCode且空值Content
*/
type CityBusAPINews2044NotModified struct {
}

func (o *CityBusAPINews2044NotModified) Error() string {
	return fmt.Sprintf("[GET /v2/Bus/News/City/{City}][%d] cityBusApiNews2044NotModified ", 304)
}

func (o *CityBusAPINews2044NotModified) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}
